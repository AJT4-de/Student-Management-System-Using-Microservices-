...
function verifyRole(requiredRoles) {
  return async (req, res, next) => {
    ...
    try {
      ...
      enrollmentServiceLogger.error(error);
      return res
        .status(403)
...
async function fetchStudentById(studentId) {
  ...
  try {
    ...
    enrollmentServiceLogger.error(
      `Error fetching student by ID ${studentId}: ${error.message}`
    );
    throw new Error(`Unable to fetch student data for ID ${studentId}`);
  }
}
...
async function fetchCourseById(courseId) {
  ...
  try {
    ...
    enrollmentServiceLogger.error(
      `Error fetching course by ID ${courseId}: ${error.message}`
    );
    throw new Error(`Unable to fetch course data for ID ${courseId}`);
  }
}
